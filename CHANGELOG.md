...
### Запросы на удаление аккаунта (тикеты)
- **Новая модель** `DeletionRequest` (таблица `deletion_requests`): id, user_id (FK CASCADE), reason (text, опционально), status (pending/approved/rejected), created_at, resolved_at
- **Backend — пользовательская часть** (`/users/me/deletion-request`):
  - `POST` — подать заявку (нельзя, если уже есть pending-заявка)
  - `GET` — посмотреть последнюю заявку
  - `DELETE` — отозвать pending-заявку
- **Backend — админская часть** (`/admin/deletion-requests`):
  - `GET` — список всех заявок с именем и email пользователя (pending идут первыми)
  - `POST /{id}/approve` — одобрить (удаляет пользователя из БД, cascade)
  - `POST /{id}/reject` — отклонить (меняет status на rejected, ставит resolved_at)
- **Frontend — ProfilePage**: раскрывающаяся секция "Удалить аккаунт" (аналогично смене пароля). Показывает форму с необязательным полем причины, либо статус pending (с кнопкой "Отозвать"), либо сообщение об отклонении с возможностью подать повторно.
- **Frontend — LobbyPage (admin panel)**: третья секция "Запросы на удаление" после таблиц пользователей и комнат. Таблица: пользователь / email / причина / дата / статус / кнопки "✓ одобрить" и "Отклонить". Одобрение открывает confirm-модал.
...
## 2026-05-16 — Доработка лендинга

**Изменения:**
- **CTA-секция**: текст подзаголовка заменён на три карточки — 🆓 Бесплатно / навсегда, 🌐 В браузере / без установки, 👥 До 50 чел. / в комнате (Вариант 2)
- **Смена темы**: кнопка 🌙/☀️ добавлена в навбар лендинга с той же view-transition анимацией, что и в остальных страницах
- **Фикс canvas**: смена темы больше не перезапускает анимацию волн — используется `themeRef` вместо зависимости в `useEffect`
- **Доска серфера**: отзеркалена (`ctx.scale(-1, 1)`) — плавник теперь у хвоста
- **Порядок секций**: поменяны местами "Возможности" (теперь 2-й слайд) и "Как это работает" (3-й слайд)

**Файлы:** `LandingPage.jsx`, `LandingPage.module.css`

---

## 2026-05-15 — Лендинг (начальный экран)

**Новое:** Добавлена страница `LandingPage` — начальный экран для неавторизованных пользователей.

**Структура лендинга:**
- Sticky-навбар с логотипом и кнопками "Войти" / "Начать бесплатно"
- Hero-секция с анимированными волнами, слоганом и двумя CTA-кнопками
- "Как это работает" — 3 шага с карточками
- "Возможности" — 6 карточек (синхронизация, чат, реакции, очередь, зал ожидания, модерация)
- Блок поддерживаемой платформы (Rutube)
- CTA-секция внизу с призывом к регистрации
- `LegalFooter` (уже существующий компонент)

**Изменения в роутинге (`App.jsx`):**
- `/` для неавторизованных → `LandingPage` (раньше редиректил на `/auth`)
- `/` для авторизованных → `LobbyPage` (без изменений)
- Добавлен компонент `HomeRoute` для разветвления

**Изменения в `AuthPage.jsx`:**
- Добавлена обработка `?tab=register` через `useSearchParams` — кнопки лендинга сразу открывают нужную вкладку

**Файлы:**
- `frontend/src/pages/LandingPage.jsx` — новый
- `frontend/src/pages/LandingPage.module.css` — новый
- `frontend/src/App.jsx` — обновлён роутинг
- `frontend/src/pages/AuthPage.jsx` — добавлен `useSearchParams`
